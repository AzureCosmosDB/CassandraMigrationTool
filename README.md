...
Streamline your migration to Azure Managed Instance for Apache Cassandra with a reliable, easy‑to‑use web app. Choose your migration tool and migration mode (offline or online). The app orchestrates bulk copy and, for online jobs, change‑feed catch‑up. You don’t need to learn or use these command-line tools yourself.

### Migration tool

- **Cassandra bulk copy**: Uses cassandra-tools. Best for moving from self-hosted or Atlas.
- **Cassandra Driver**: Uses the driver for bulk copy and change feed catch-up.
- **Cassandra Driver (Cosmos DB Cassandra API read optimized)**: Optimized for Cosmos DB Cassandra API as source. Uses RU-friendly read patterns.

### Migration mode

- **Offline**: Snapshot-only copy. Job completes automatically when copy finishes.
- **Online**: Copies bulk data, then processes change feed to catch up. Requires manual Cut Over to finish.
...
- [How to Use](#how-to-use)
  ...
  - [Dynamic Scaling of Bulk Copy Workers](#dynamic-scaling-of-bulk-copy-workers)
...
- [Tables input formats](#tables-input-formats)
...
- [RU-optimized copy (Cosmos DB Cassandra API source)](#ru-optimized-copy-cosmos-db-cassandra-api-source)
...
- **Flexible Migration Options**  
  Supports both online and offline migrations to suit your business requirements. It supports multiple tools to read data from the source and write it to the target. If you are migrating from on-premises Cassandra cluster, consider using the [On-Premises Deployment](#on-premises-deployment) to install the app locally and transfer data to Azure Managed Instance for Apache Cassandra. This eliminates the need to set up an Azure VPN.

- **User-Friendly Interface**  
  No steep learning curve—simply provide your connection strings and specify the tables to migrate.
...
Effortlessly migrate your Cassandra tables while maintaining control, security, and scalability. Begin your migration today and unlock the full potential of Azure Cosmos DB Cassandra API!

## Azure Deployment Options

The Cassandra Migration Utility can be deployed to Azure using two different options, each optimized for different workload requirements:
...
**Best for**: Migrating from on-premises Cassandra clusters directly to Azure Managed Instance for Apache Cassandra

- ✅ Eliminates need for Azure VPN setup
- ✅ Direct local access to source Cassandra
...
Before migrating your data, it's essential to prepare your target Azure Managed Instance for Apache Cassandra environment with the proper schema structure. The **Schema Migration Tool** (located in the `/SchemaMigration` folder) automates this critical preparation step.

#### What It Does

The Schema Migration Tool analyzes your source Cassandra tables and intelligently transforms them for optimal performance on Azure Managed Instance for Apache Cassandra. It handles:

- **Table Creation**: Automatically creates target tables with appropriate configurations
...
- **Partition Key Migration**: Optionally migrates partitioning configurations for distributed tables
- **Index Optimization**: Identifies and removes redundant compound indexes to improve performance
- **Table Colocation**: Groups related tables together for better query performance
...
2. **Schema Compatibility**: Azure Managed Instance for Apache Cassandra may have different schema requirements or optimizations compared to your source Cassandra. The tool ensures your target schema is properly configured before data arrives.

3. **Partitioning Configuration**: If your tables are partitioned, the partition key must be defined before any data is written to the table. You cannot add or modify partition keys after data insertion.
...
1. ✅ **Schema Migration**: Run the Schema Migration Tool to prepare your Azure Managed Instance for Apache Cassandra environment
2. ✅ **Verify Schema**: Confirm all tables, indexes, and partition keys are correctly created
...
> **Important**: When running data migration after schema migration, ensure you enable **Append Mode** in the Migration Web App. This preserves the tables, indexes, and partition keys created by the Schema Migration Tool. Without Append Mode, the data migration tool may drop and recreate tables, losing your optimized schema configuration.

### Add a New Job

1. Go to the home page. Click **New Job**.  
2. In the **New Job Details** pop-up, enter all required information.  
3. Refer to [Tables input formats](#tables-input-formats) to learn multiple ways to input table list for migration. If necessary, use the [list table steps](#get-list-of-tables) to create a comma-separated list of table names.  
4. Choose the migration tool: either **Cassandra Bulk Copy** or **Cassandra Driver**.  
5. Select the desired [migration mode](#migration-modes).
1. Select **Append Mode** to preserve existing table(s) on the target without deleting them.  
...
**Note for Azure Web App deployments:** For the Cassandra Bulk Copy option, the Web App will download the cassandra-tools from the URL specified in the Web App settings. Ensure that the Web App has access to this URL. If the Web App does not have internet access, you can download the cassandra-tools zip file to your development machine, then copy it to the wwwroot folder inside the published folder before compressing it. Afterward, update the URL in the Web App settings to point to the Web App's URL (e.g., https://<WebAppName>.azurewebsites.net/<zipfilename.zip>). This note does not apply to Azure Container Apps (ACA) deployments, where cassandra-tools are pre-installed in the container image.
...
- **Online Migration**: Apart from the bulk data copy activity done in the offline migration, a change feed monitors all additions/updates/deletes. After the bulk data copy is completed, the data in the change feed is copied to the target to ensure that all updates made during the migration process are also transferred to the target. The application downtime required is minimal.


#### Change feed retention

For online jobs, ensure that the change feed retention of the source Cassandra is large enough to store operations for at least the duration of both the download and upload activities. If the change feed retention is too small and there is a high volume of write operations, the online migration may fail or be unable to read all documents from the change feed in time.
...
The job processes tables in the order they are added. Since larger tables take more time to migrate, it’s best to arrange the tables in descending order of their size or document count.

### View a Job

1. From the home page  
2. Select the **eye icon** corresponding to the job you want to view.  
3. On the **Job Details** page, you will see the tables to be migrated and their status in a tabular format.
...
1. Depending on the job's status, one or more of the following buttons will be visible:  
   ...
   - **Update Tables**: Select this to add/remove tables in the current job. You can only update tables for a paused job. Removing a table that is partially or fully migrated will lead to the loss of its migration details, and you will need to remigrate it from the start.  
   - **Cut Over**: Select this to cut over an online job when the source and target are completely synced. Before cutting over, ensure to stop write traffic to the source and wait for the Time Since Last Change to become zero for all tables. Once cut over is performed, there is no rollback.  
...
- Check if the transactions per second on the source are very high; in this case, you may need a larger app service plan or a dedicated web app for the table.
...
Time Since Last Batch displays the time elapsed since the change feed was last checked for updates. This metric is shown in the Job Viewer UI as **CS Last Checked** and indicates how recently the change feed processor examined the table for new changes.

This value helps monitor the health and activity of the change feed processing:

- A small value (e.g., seconds or a few minutes) indicates active monitoring and regular checks of the change feed.
- A large or increasing value may suggest the change feed processor is not running, the job is paused, or there's an issue preventing regular checks.
- If this value is "NA", it means the change feed has not yet been initialized or checked for this table.

This is different from "Time Since Last Change" which shows when the last actual change was processed, whereas "Time Since Last Batch" shows when the change feed was last polled for updates.
...
- ObjectId Partitioner
    - Choose the partitioning method for ObjectId-based tables:
      - **Use Sample Command**: Uses Cassandra's $sample command (recommended for most workloads)
...
- **Cut Over**: For online jobs, enabled when change feed lag reaches zero for all tables. You can choose Cut Over with or without Sync Back.
- **Update Tables**: Add/remove tables on a paused job. Removing a table discards its migration and change feed state.
- **Reset Change Stream**: For selected tables, reset the checkpoint to reprocess from the beginning. Useful if you suspect missed events.
...
### Dynamic Scaling of Bulk Copy Workers
...
The tool uses three types of workers during Bulk Copy migration:

1. **Dump Workers**: Control how many `bulk copy` processes run in parallel to extract data from the source
2. **Restore Workers**: Control how many `bulk copy` processes run in parallel to load data into the target
3. **Insertion Workers**: Control the `--numInsertionWorkersPerCollection` parameter for each `bulk copy` process
...
- You have many small tables or chunks remaining
- Keyspace throughput on source/target can handle higher load

**When to Decrease Workers**:
- System resources are constrained (high CPU/memory)
- Keyspace is experiencing throttling or high latency
...
- **Insertion Workers**: Default is ProcessorCount/2; increase to 8-12 for large tables with high write capacity
...
- Change feed resume from last saved token
...
- For tables where dump completed during pause, resume will trigger restore automatically
...
When migrating large tables, the tool **splits the table into chunks** based on the `_id` field range. This enables:
...
- Ability to retry failed chunks without re-processing entire table
...
- Partitions based on time ranges distributed evenly across the table's time span
...
**Best For**:
- Tables where documents are primarily inserted chronologically
- Time-series data (logs, events, transactions)
- Tables with steady insert rate over time
- Large tables (millions of documents) where sampling would be expensive
...
**Best For**:
- Tables with non-uniform write patterns (bursts of activity)
...
- Creates boundaries by paginating through the table (e.g., every 100K records)
- Avoids loading all documents into memory

**Benefits**:
- **Equal chunk sizes**: Each chunk has the same number of records
- **Memory efficient**: Progressive filtering instead of full table scan
...
**Best For**:
- Tables with random or unpredictable ObjectId patterns
- When you need exact control over chunk size
- Tables where time-based approaches produce imbalanced chunks
- Debugging scenarios requiring predictable record counts per chunk

##### 4. **Use Sample Command** (Fallback for small tables)

**How it Works**:
- Uses Cassandra's `$sample` aggregation command to randomly sample ObjectIds
- Sorts sampled values and creates boundaries
- Quick and simple approach for smaller datasets

**Benefits**:
- **Simple implementation**: Leverages Cassandra's built-in sampling
- **Low overhead**: Single aggregation query
- **Works universally**: No assumptions about data distribution

**Best For**:
- Small to medium tables (< 1M documents)
- Quick migrations where optimization isn't critical
- Tables where other methods fail or timeout
- Development/testing scenarios

#### Specifying DataType for _id: Why It Matters

When you specify `"DataTypeFor_Id": "ObjectId"` in the table configuration, you unlock significant performance benefits:
...
**Behavior**:
- Tool must scan table to determine `_id` data type
...
- **Skips type detection overhead** (saves 1-2 queries per table)
...
- **Keyspace efficiency**: Better index utilization, fewer type coercion operations
...
|----------|----------|----------------------|
| **ObjectId** | Standard Cassandra tables | Timestamp-based or random sampling |
...
- Table has more than 1 million documents
- You know the `_id` field type is consistent
- Performance is critical (large-scale migrations)
- Table uses ObjectId for `_id` (most common case)

**Let Tool Auto-Detect When**:
- Table has mixed `_id` types (rare, but possible)
- Small tables (< 100K documents) where overhead is negligible
...
- Tables with ~1M docs/month: Use monthly chunks (12 chunks/year)
- Tables with ~1M docs/week: Use weekly chunks (52 chunks/year)
- Tables with ~1M docs/day: Use daily chunks (365 chunks/year)
...
- Migration tool
    - Cassandra bulk copy: Uses cassandra-tools. Best for moving from self-hosted or Atlas to Cosmos DB Cassandra API. Requires access to Cassandra Tools ZIP URL configured in Settings.
    - Cassandra Driver: Uses the driver for bulk copy and change feed catch-up.
    - Cassandra Driver (Cosmos DB Cassandra API read optimized): Optimized for Cosmos DB Cassandra API as source. Skips index creation and uses RU-friendly read patterns. Filters on tables are not supported for this mode.
...
- Append Mode
    - If ON: Keeps existing target data and appends new documents. No table drop. Good for incremental top-ups.
    - If OFF: Target tables are overwritten. A confirmation checkbox is required to proceed.
...
- Change Feed Modes
    - Delayed: Start change feed processing after all tables are completed. This mode is ideal when migrating a large number of tables.
    - Immediate: Start change feed processing immediately as each table is processed.
    - Aggressive: Use aggressive change feed processing when the change feed is small or the write rate is very high. Avoid this mode if a large number of tables need to be migrated.
...
- All tables use ObjectId for the _id field
    - If ON: Automatically applies `"DataTypeFor_Id": "ObjectId"` to all tables in the job, enabling ObjectId-specific optimizations.
    - Benefits: Faster partitioning (6x speedup), optimized chunk boundaries, leverages timestamp-based partitioning strategies.
    - If OFF: Each table's _id type is auto-detected, or you can specify DataTypeFor_Id individually via JSON format.
...
You can specify tables in three ways:

1) CSV list
- Example: `ks1.tbl1,ks1.tbl2,ks2.tblA`
- Order matters. Larger tables should appear first to reduce overall time.
- 
2) CSV list with wildcards
- Example: `ks1.*,*.users,*.*`
- If the table count is large consider splitting it into multiple jobs.
...
- Filters must be valid Cassandra query JSON (as a string). Only supports basic operators (`eq`,`lt`,`lte`,`gt`,`gte`,`in`) on root fields. They apply to both bulk copy and change feed.
- Specify DataTypeFor_Id if the table contains only a single data type for the _id field. Supported values are: ObjectId, Int, Int64, Decimal128, Date, BinData, String, and Object.
...
## RU-optimized copy (Cosmos DB Cassandra API source)

- Reduce RU consumption when reading from Cosmos DB Cassandra API by using change-feed-like incremental reads and partition batching. This method skips index creation during the copy; use the schema migration script at https://aka.ms/mongoruschemamigrationscript to create the indexes on the target tables. It focuses on efficient incremental ingestion. Table filters are not supported.
...
- Choose an appropriate App Service plan (P2v3 recommended for large or high-TPS workloads). You can dedicate a web app per large table.
- For driver copy, tune “Cassandra driver page size” upward for higher throughput, but watch memory and target write capacity.
- For online jobs, ensure change feed retention on the source is large enough to cover full bulk copy duration plus catch-up.
