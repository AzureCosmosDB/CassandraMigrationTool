...
| **Change Feed Replay** | Continuously reads Cosmos DB Cassandra change feed and replays inserts and updates to the target | None |
...
- Change feed is built in to the Cosmos DB Cassandra API —
  no special table creation or enablement is needed for
  online migration.
...
- Starts polling the Cosmos DB Cassandra change feed
  (using `COSMOS_CHANGEFEED_START_TIME()`)
...
|----------|-----------------|
| Low-traffic app (online mode) | **< 30 seconds** |
| High-traffic app (online mode) | **1–5 minutes** |
| Offline mode only | Duration of bulk copy (no live sync) |
...
**Q: Does online mode capture deletes?**
A: No. The regular Cosmos DB Cassandra change feed
captures inserts and updates but does not capture
deletes. If your application deletes rows during
migration, you will need to handle those manually
after cutover.

---

*Document version: 1.2 — March 2026*
*Updated: Online mode uses regular Cosmos DB Cassandra
change feed — no special enablement required.*
