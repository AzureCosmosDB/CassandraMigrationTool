...
| **Online Migration** | Bulk copy + live sync via Cosmos DB change feed |
...
### 3.4 For Online Migration (Change Feed)

The Cosmos DB Cassandra API change feed is available on
all tables by default. No special table creation options
are required.
...
copy, using the Cosmos DB change feed.
...
> ⚠️ All tables selected for online migration must be
> accessible via the Cosmos DB change feed.
