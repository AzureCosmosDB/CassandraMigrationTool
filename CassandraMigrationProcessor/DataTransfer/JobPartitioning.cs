...
internal sealed class JobPartitioning
{
    ...
    public IReadOnlyCollection<string> FailedUnitIds { get; }

    public JobPartitioning(
        IReadOnlyList<TablePartitioning> chunks,
        IReadOnlyCollection<string>? failedUnitIds = null)
    {
        Chunks = chunks;
        AllPartitions = chunks.SelectMany(c => c.Partitions).ToList();
        FailedUnitIds = failedUnitIds ?? Array.Empty<string>();
    }
